...
from utils.dataloader import datainfo, dataload,CIFARCDataset
import torchvision.transforms as transforms
from colorama import Fore, Style
from utils.sampler import RASampler
from utils.mix import cutmix_data, mixup_data, mixup_criterion, cutout,same_class_mixup_data,Intensity_Mixup,MagnitudeMix, MagALLMix, MagRandomMix,LabelMix_Mixup
from utils.losses import LabelSmoothingLoss,ECE,AECE
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mixup', type=str, default='baseline', help='mixup augmentation')
    ...
    parser.add_argument('--calib', type=str, default='ce', help='calib augmentation')
    parser.add_argument('--corruption_type', type=str, default='brightness', help='calib augmentation')
    ...
    data_dir_c10 = '/mnt/data/MagMix/Calib-Mix-ooD/data-ood/CIFAR-10-C/'
    data_dir_c100 = '/mnt/data/MagMix/Calib-Mix-ooD/data-ood/CIFAR-100-C/'
    data_dir_tim = '/mnt/data/MagMix/Calib-Mix-ooD/data-ood/Tiny-ImageNet-C/'

    transformc = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010))
    ])
    cifar10c_dataset = CIFARCDataset(data_dir_c10, args.corruption_type, transform=transformc)
    cifar10c_loader = torch.utils.data.DataLoader(cifar10c_dataset, batch_size=args.batch_size, shuffle=False, pin_memory=True)
    cifar100c_dataset = CIFARCDataset(data_dir_c100, args.corruption_type, transform=transformc)
    cifar100c_loader = torch.utils.data.DataLoader(cifar10c_dataset, batch_size=args.batch_size, shuffle=False, pin_memory=True)
    ...
    if args.model == 'vit':
        if args.pretrained_model_path is not None:
            ...
            if args.magmix:
                ...
                writer = SummaryWriter(os.path.join('logs-mix', args.dataset, args.mixup+'-scratch-cls-'+args.calib+'-bz'+str(args.batch_size)))
            save_path = os.path.join(os.getcwd(), 'save-scratch',args.calib, args.mixup)
            if save_path:
                os.makedirs(save_path, exist_ok=True)
        ...
        if args.magmix:
            ...
            writer = SummaryWriter(os.path.join('logs-swin-mix', args.dataset, args.mixup+'-scratch-cls-'+args.calib+'-bz'+str(args.batch_size)))
        save_path = os.path.join(os.getcwd(), 'save-scratch-swin',args.calib, args.mixup)
        if save_path:
                os.makedirs(save_path, exist_ok=True)
        ...
        if args.magmix:
            ...
            writer = SummaryWriter(os.path.join('logs-res-mixup', args.dataset, args.mixup+'-scratch-cls-'+args.calib+'-bz'+str(args.batch_size)))
        save_path = os.path.join(os.getcwd(), 'save-scratch-res',args.calib, args.mixup)
        if save_path:
                os.makedirs(save_path, exist_ok=True)
